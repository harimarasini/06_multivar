...
lm_dayl.prcp <- lm(strength_gtex ~ mean_dayl.s_Apr + 
  ...
  tidy() %>%
  filter(term != "(Intercept)") %>%
  mutate(mod = "lm_2var.uncor")   # 2 variable model with uncorrelated predictors
...
lm_dayl.apr %>%
  ...
  # if we want to change the order of levels, we need to rearrange this section. 
  ...
  geom_pointrange(aes(y = estimate, 
                      ymin = estimate - std.error, #use this function-type for any estimates including similar relationships
...
**ANS:**
- For model 1 and model with uncorrelated variable, magnitude and direction didn't change, no relationship

- magnitude and direction changes when model includes variable with highly correlated variable (multicollinearity is seen)
- error around the estimate highly changed
...
Redundant predictors in models will make model complex, time-consuming as well as bring wrong prediction, so we need to remove them (think of machine learning models)
...
- Is an **unsupervised analysis** (no outcome, i.e., there are only X-variables, no Y-variables)
...
# Scree plot , helps visualizing the % of variance explained by PCs
...
## Inspecting PCs  
Lets inspect PC1 through the loadings (weights) of each variable towards it.  
...
weather_postpca <- weather %>% 
  bind_cols(pca_scores)       #binds cols form pca_scores column 
...
**WHY PCA**
**- to reduce dimensionality and to check if PCs are orthogonal (or to check multicollinerality)**
